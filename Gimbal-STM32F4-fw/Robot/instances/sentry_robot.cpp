...
void SentryRobot::InitAllActuators(void)
{
    ...
    gimbal_motor[GIMBAL_FIRST_PITCH_MOTOR] = new GM6020(CAN2, GIMBAL_FIRST_PITCH_MOTOR_ID, GIMBAL_MOTOR_REDUCTION_RATIO);
    ...
    gimbal_motor[GIMBAL_SECOND_YAW_MOTOR] = new GM6020(CAN2, GIMBAL_SECOND_YAW_MOTOR_ID, GIMBAL_MOTOR_REDUCTION_RATIO);
    ...
    gimbal_motor[GIMBAL_SECOND_YAW_MOTOR]->m_kalman_filter_angle = new Kalman(1, 0.001f, 0.0001f,0.001f, 0.1f);
...
void SentryRobot::SendControlCommand(void)
{
    ...
    if(G_system_monitor.CAN1_rx_fps < 1500)
    {
         for (int i = 0; i < SHOOT_MOTOR_NUM; i++)
         {
    ...
    if(G_system_monitor.CAN2_rx_fps < 1500)
    {
         for (int i = 0; i < GIMBAL_MOTOR_NUM; i++)
         {
    ...
    G_gun_shift.Shift(G_gun_shift.m_target_angle);
}