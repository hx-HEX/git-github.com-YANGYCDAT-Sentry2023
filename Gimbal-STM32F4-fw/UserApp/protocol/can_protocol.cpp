    ...
    if(context->CANx == CAN1){
        ...
        switch(context->CANx_RxMsg.StdId)
        {
           case (0x204 + GIMBAL_FIRST_PITCH_MOTOR_ID):	
                G_sentry.gimbal_motor[SentryRobot::GIMBAL_FIRST_PITCH_MOTOR]->
                AngleUpdate(GetEncoderNumber(&(context->CANx_RxMsg.Data[0])));
                G_sentry.gimbal_motor[SentryRobot::GIMBAL_FIRST_PITCH_MOTOR]->
                SpeedUpdate(GetMotorSpeed(&(context->CANx_RxMsg.Data[0])));
                G_sentry.gimbal_motor[SentryRobot::GIMBAL_FIRST_PITCH_MOTOR]->m_state_update_times++;
                break;              
            case (0x204 + GIMBAL_SECOND_YAW_MOTOR_ID):	
                G_sentry.gimbal_motor[SentryRobot::GIMBAL_SECOND_YAW_MOTOR]->
                AngleUpdate(GetEncoderNumber(&(context->CANx_RxMsg.Data[0])));
                G_sentry.gimbal_motor[SentryRobot::GIMBAL_SECOND_YAW_MOTOR]->
                SpeedUpdate(GetMotorSpeed(&(context->CANx_RxMsg.Data[0])));
                G_sentry.gimbal_motor[SentryRobot::GIMBAL_SECOND_YAW_MOTOR]->m_state_update_times++;
                break;    
        }
    }
}
